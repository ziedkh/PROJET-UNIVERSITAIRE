...
%token inf infegal supergal egal diff sup  supegal
%token accouv accfer
%token fdf
%start FICHIER
%%
...
if (argc <1) printf("vous devez inclure au moins une fichier dans la ligne de commande");
for(int i=1 ; i< argc; i++){
printf("\n\n[*]Fichier %s  :\n",argv[i]);
yyin=fopen(argv[i],"r");
if (yyin>0)printf("       [*]fichier %d : DISPONIBLE\n" , i );
